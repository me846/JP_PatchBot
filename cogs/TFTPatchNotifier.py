...
def scrape_patch_notes(patch_number):
    ...
    url = f"https://www.leagueoflegends.com/ja-jp/news/game-updates/teamfight-tactics-patch-{formatted_patch_number}-notes/"
...
class TFTPatchNotes(commands.Cog):
    def __init__(self, bot):
        self.bot = bot

    @commands.command(name='TfPatchUP')
    @commands.is_owner()
    async def tfpatch_up(self, ctx, patch_number: str = None):
        if not patch_number:
        ...
        # すべての通知設定を取得
        c.execute('SELECT guild_id, channel_id, notification_type FROM notifications WHERE notification_type IN ("tft", "both")')
        ...
        for guild_id, channel_id, _ in notifications:
            ...
            if guild:
                ...
                if channel:
                    ...
                    full_notes_url = f"https://www.leagueoflegends.com/ja-jp/news/game-updates/teamfight-tactics-patch-{patch_number.replace('.', '-')}-notes/"
                    embed.add_field(name="全文を読む", value=f"[こちらをクリック]({full_notes_url})", inline=False)
    ...
    @commands.command(name='TfTestUP')
    @commands.is_owner()
    async def tftest_up(self, ctx, patch_number: str = None):
        if not patch_number:
            await ctx.send('パッチナンバーを指定してください。')
            return
...
async def setup(bot):
    await bot.add_cog(TFTPatchNotes(bot))